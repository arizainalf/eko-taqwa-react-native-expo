...
import { useEkoKaidahChatData } from 'hooks/ekoKaidah/useAiChatKaidah'
...
const STORAGE_KEY_PREFIX = 'ai_kaidah_usage_' // tambahkan prefix
...
export default function AiChatEkoCpScreen() {
    ...
    const { messages, loading, sendMessage, isSending } = useEkoKaidahChatData(
        temaId,
        deviceId ?? ''
    )

    const [jenis, setJenis] = useState<'ushuliyah' | 'fiqhiyah'>('ushuliyah')
    ...
    const handleSend = useCallback(
        async (jenisPesan: 'ushuliyah' | 'fiqhiyah') => {
            if (isSending) return
            const now = Date.now()
    ...
    return (
        <View className="flex-1 bg-white">
                ...
                <View className="flex-row items-center mt-5">
                    ...
                    <View>
                        ...
                        <Text className="text-base text-green-100">
                            Memuat Data Eko Kaidah dengan AI
                        </Text>
                    </View>
                </View>
            </View>
            ...
            <View className="absolute bottom-4 left-4 flex-row gap-3">
                <Pressable
                    onPress={() => setJenis('ushuliyah')}
                    className={`px-4 py-2 rounded-full border ${jenis === 'ushuliyah'
                            ? 'bg-green-600 border-green-600'
                            : 'bg-white border-gray-300'
                        }`}
                >
                    <Text
                        className={`text-sm ${jenis === 'ushuliyah' ? 'text-white' : 'text-gray-700'
                            }`}
                    >
                        Ushuliyah
                    </Text>
                ...
                <Pressable
                    onPress={() => setJenis('fiqhiyah')}
                    className={`px-4 py-2 rounded-full border ${jenis === 'fiqhiyah'
                            ? 'bg-green-600 border-green-600'
                            : 'bg-white border-gray-300'
                        }`}
                >
                    <Text
                        className={`text-sm ${jenis === 'fiqhiyah' ? 'text-white' : 'text-gray-700'
                            }`}
                    >
                        Fiqhiyah
                    </Text>
