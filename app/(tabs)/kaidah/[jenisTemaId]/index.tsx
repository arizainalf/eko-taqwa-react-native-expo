...
import { useTemaKaidahData } from 'hooks/ekoKaidah/useTemaKaidahData'
...
export default function TemaList() {
    ...
    const { data, loading, error, refetch } = useTemaKaidahData(jenisTemaId)
    ...
    return (
        ...
        >
            ...
            <View className="p-5 mx-3 bg-white -mt-64 rounded-2xl min-h-[200px]">
                ...
                {/* <InfoCardFull title={'Total Video'} value={data!.kaidah_count} icon={'youtube'} /> */}
                ...
                {data?.tema.map((tema) => (
                    <ItemCard
                        key={tema.id}
                        title={tema.nama}
                        value={`${tema.kaidah_count} Kaidah`}
                        href={`/kaidah/${jenisTemaId}/${tema.id}`}
                    />
                ))}
