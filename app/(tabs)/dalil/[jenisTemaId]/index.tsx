...
export default function TemaList() {
    ...
    const { data, loading, error, refetch } = useTemaDalilData(jenisTemaId)
            ...
            <View className="p-5 mx-3 bg-white -mt-64 rounded-2xl min-h-[200px]">
                ...
                {data?.tema.map((tema) => (
                    <ItemCard
                        key={tema.id}
                        title={tema.nama}
                        value={`${tema.dalil_count} Ayat & Hadist`}
                        href={`/dalil/${jenisTemaId}/${tema.id}`}
                    />
                ))}
                ...
                {data?.tema.length === 0 && (
                    <View className="items-center py-10 px-5">
                        ...
                        <Text className="text-lg font-bold text-gray-900 mt-4 mb-2">
                            Tidak Ada Data Eko Ayat Hadist
                        </Text>
                    </View>
                )}
            </View>
