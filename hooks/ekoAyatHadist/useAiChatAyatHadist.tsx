...
export function useEkoAyatHadistChatData(temaId: string, deviceId: string) {
    const [messages, setMessages] = useState<ChatMessage[]>([])
        ...
        const fetchMessages = useCallback(async () => {
            ...
            try {
                const data = await apiGet<any[]>(`/v1/ayat_hadist/tema/${temaId}/chat/${deviceId}`)
        ...
        const sendMessage = useCallback(async (jenis: string) => {
            ...
            try {
                const result = await apiGet<{ reply?: string }>(
                    `/v1/ayat_hadist/tema/${temaId}/chat/${deviceId}/send-message/${jenis}`
                )
